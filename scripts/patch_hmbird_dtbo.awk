...
function overlay_has_gpio(overlay_start, overlay_end, gpio_start, gpio_end,
    ...
    if (!gpio_end)
        return 0
    # Some production overlays carry the compatible property and some expose
    # only the canonical node name plus pinctrl properties.  The direct-child
    # name is the stable structural anchor; requiring compatible rejected
    # valid EXT entries on the latter form.
    return 1
}
